...
function onConnect() {
    ...
    mqttclient.subscribe('/sensors/#', function(err) {
		if (!err) {
			console.log("mqtt","subscribed to topic /sensors/#");
		} else {
...
function onMessage(topic, msg) {
    try {
        ...
        $('.card#distance_surface .value').html(numeral(Number(msgdata.data.distance_surface)).format('0') + '%');
        $('.card#distance_surface').attr('data-status',msgdata.data.tank_limit);
